...
cp -r "$FRAMEWORK_SOURCE/templates/"* "$TARGET_DIR/templates/" 2>/dev/null || echo "    (No templates found in source, skipping)"
cp -r "$FRAMEWORK_SOURCE/memory/rules/"* "$TARGET_DIR/memory/rules/" 2>/dev/null || echo "    (No rules found in source, skipping)"
...
cp "$FRAMEWORK_SOURCE/memory/constitutional-framework.md" "$TARGET_DIR/memory/" 2>/dev/null || echo "# Constitutional Framework" > "$TARGET_DIR/memory/constitutional-framework.md"
